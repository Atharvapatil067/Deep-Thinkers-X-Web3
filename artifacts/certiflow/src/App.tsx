...
function App() {
  ...
  const isAuthorizedIssuer = Boolean(walletAddress && issuers.some((issuer) => issuer.status === 'Active' && issuer.wallet.toLowerCase() === walletAddress.toLowerCase()));
  ...
  return <div className="dark cf-noise min-h-[100dvh] bg-[#0b0f22]"><BrowserRouter basename={import.meta.env.BASE_URL}><Navbar walletConnected={walletConnected} walletAddress={walletAddress} walletChainId={walletChainId} isMetaMaskAvailable={isMetaMaskAvailable} isConnecting={isConnecting} walletError={walletError} onConnect={connectWallet} onDisconnect={disconnectWallet} /><Routes><Route path="/" element={<LandingPage walletConnected={walletConnected} onConnect={connectWallet} onDisconnect={disconnectWallet} />} /><Route path="/verify" element={<VerifyPage certificates={certificates} />} /><Route path="/issuer" element={<IssuerPage walletConnected={walletConnected} walletAddress={walletAddress} isSepolia={isSepolia} isAuthorizedIssuer={isAuthorizedIssuer} walletChainId={walletChainId} isMetaMaskAvailable={isMetaMaskAvailable} isConnecting={isConnecting} onConnect={connectWallet} onDisconnect={disconnectWallet} certificates={certificates} onMint={mintCertificate} />} /><Route path="/credentials" element={<CredentialsPage walletConnected={walletConnected} walletAddress={walletAddress} walletChainId={walletChainId} isMetaMaskAvailable={isMetaMaskAvailable} isConnecting={isConnecting} onConnect={connectWallet} onDisconnect={disconnectWallet} certificates={certificates} />} /><Route path="/admin" element={<AdminPage issuers={issuers} walletAddress={walletAddress} onConnect={connectWallet} onAdd={addIssuer} onRevoke={revokeIssuer} />} /><Route path="*" element={<NotFound />} /></Routes><Toaster position="bottom-right" toastOptions={{ style: { background: '#171f43', color: '#e9f1ff', border: '1px solid rgba(130,150,232,.2)', fontSize: '13px' } }} /></BrowserRouter></div>;
}
